...
This directory holds paired request/response fixtures that any Tessera implementation MUST pass to claim "Tessera v0.0.1 support."
...
4. **Run all fixtures in sequence in a single test**: create → get → update_status. The fixtures form a coherent narrative; running them out of order will fail because each assumes the prior state.

## What "passes" means

An implementation passes Tessera v0.0.1 if:
